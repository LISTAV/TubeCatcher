...
class YTDLPGUI:
        ...
        try:
            with YoutubeDL({'quiet': True, 'no_warnings': True, 'noplaylist':True}) as ydl:
                ...
                for f in info.get('formats', []):
                    ...
                    self.all_formats.append((f['format_id'], format_str, is_audio_only, is_video_only))
                
                # Add a "best video + best audio" option for non-audio-only downloads
                if not self.audio_only_var.get():
                    self.all_formats.insert(0, ('bestvideo+bestaudio/best', 'Best Video + Audio', False, False))
    ...
    def filter_formats(self):
        if self.audio_only_var.get():
            ...
            self.current_formats = [f for f in self.all_formats if not f[2] or f[0] == 'bestvideo+bestaudio/best']  # Video or video+audio formats
    ...
    def check_dependencies(self):
        ...
        try:
            ...
            self.log_message("FFmpeg found: " + result.stdout.split('\n')[0])
        except FileNotFoundError:
            messagebox.showerror(
                "FFmpeg Missing",
                "FFmpeg is required for audio/video merging and MP3 conversion.\n"
                "Please download FFmpeg from https://ffmpeg.org/ and add it to your system PATH.\n"
                "On Windows, ensure ffmpeg.exe is in a directory listed in your PATH environment variable."
            )
    ...
    def start_download(self):
        ...
        # Set output template based on audio-only or video
        outtmpl = os.path.join(self.output_var.get(), '%(title)s.%(ext)s')
        if self.audio_only_var.get():
            outtmpl = os.path.join(self.output_var.get(), '%(title)s.mp3')

        download_format = format_id
        if not self.audio_only_var.get() and is_video_only:
        ...
        if self.audio_only_var.get():
            ...
            options['merge_output_format'] = 'mp4'  # For video downloads
